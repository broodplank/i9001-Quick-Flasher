...
;Static paths!
FileInstall("C:\Users\Flo\Desktop\flasher\img\bar.jpg", @TempDir & "\bar.jpg", 1)
FileInstall("C:\Users\Flo\Desktop\flasher\img\barv.jpg", @TempDir & "\barv.jpg", 1)


;Create GUI
$GUI = GUICreate("i9001 Quick Flasher", 560, 535)
GUISetFont(8, 8, 0, "Verdana", $GUI)

GUICtrlCreateLabel("Flashing Log:", 305, 5)
$log = GUICtrlCreateEdit("-------------------------------------------" & @CRLF & "i9001 Quick Flasher v1.4 initiated..", 305, 22, 251, 475, BitOR($WS_VSCROLL, $ES_AUTOVSCROLL, $ES_MULTILINE, $ES_READONLY))
...
GUICtrlCreateLabel("Partition: boot.img (Kernel)" & @CRLF & "Location: /dev/block/mmcblk0p8", 5, 5)
...
GUICtrlCreateLabel("Partition: recovery.img (Recovery)" & @CRLF & "Location: /dev/block/mmcblk0p13", 5, 115)
...
GUICtrlCreateLabel("Partition: system.img.ext4 (ROM Base)" & @CRLF & "Location: /dev/block/mmcblk0p15", 5, 225)
...
$partition = GUICtrlCreateCombo("mmcblk0p12 (persist)", 5, 390, 175, 18, $CBS_DROPDOWNLIST)
...
$copyright = GUICtrlCreateLabel("©2012 broodplank.net - Use with caution! I am not responsible for damage done to your device.", 5, 520)
...
_AddLog(@CRLF & "<<< i9001 USB Connection >>>")
...
If FileReadLine(@ScriptDir & "\state", 3) = "unknown" Or FileReadLine(@ScriptDir & "\state", 3) = "offline" Or FileReadLine(@ScriptDir & "\state", 3) = "bootloader" Then
	_AddLog("[ ] i9001 USB Connection")
	_AddLog(@CRLF & "Your i9001 is not connected or unreachable! Please read the instructions for reference.")
	GUICtrlSetData($progress, 0)
Else
	_AddLog("[X] i9001 USB Connection")
	...
	Else
		...
		If @error Then
			GUICtrlSetData($progress, 0)
			MsgBox(0, "i9001 Quick Flasher", "Unknown ADB error, Exiting!")
			Exit
		EndIf
...
While 1
	...
	Select
		...
		Case $msg = $help
			MsgBox(0, "i9001 Quick Flasher", "Instructions:" & @CRLF & @CRLF & "1. Connect your i9001 in debugging mode to the PC using the USB cable" & @CRLF & "( Settings > Applications > Development > USB Debugging )" & @CRLF & @CRLF & "2. Collect the files that you want to flash to your phone" & @CRLF & "(for example you want to install a kernel and recovery)" & @CRLF & @CRLF & "3. Check the partition boxes that you want to flash." & @CRLF & @CRLF & "4. Load the images (boot.img and recovery.img in this case) by clicking the  " & Chr(34) & "Browse..." & Chr(34) & " button" & @CRLF & @CRLF & "5. As additional option you can choose to wipe the partition before flashing it." & @CRLF & @CRLF & "6. Click " & Chr(34) & "Start Now" & Chr(34) & " to start the flashing process" & @CRLF & @CRLF & "7. Sit back and wait until your i9001 reboots, now you're done :)" & @CRLF & @CRLF & "Please Note: This tool is not some tool to screw around with!" & @CRLF & "Flashing a kernel from another phone would lead to an unrepairable hard brick!" & @CRLF & "So please take care at all times when using i9001 Quick Flasher")


		Case $msg = $about
			MsgBox(0, "i9001 Quick Flasher", "About:" & @CRLF & @CRLF & "i9001 Quick Flasher v1.4 " & @CRLF & "Coded by broodplank(1337)" & @CRLF & @CRLF & "Official Site: wwww.broodplank.net/quickflasher.php" & @CRLF & "XDA Topic: forum.xda-developers.com/showthread.php?t=1574149" & @CRLF & "Donate Page: www.broodplank.net/donate.html")

		Case $msg = $browse_bootimg
			$getpath_bootimg = FileOpenDialog("i9001 Quick Flasher", @WorkingDir, "Image Files (*.img)", 1, "boot.img")
			GUICtrlSetData($path_bootimg, $getpath_bootimg)

		Case $msg = $browse_recovery
			$getpath_recovery = FileOpenDialog("i9001 Quick Flasher", @WorkingDir, "Image Files (*.img)", 1, "recovery.img")
			GUICtrlSetData($path_recovery, $getpath_recovery)

		Case $msg = $browse_system
			$getpath_system = FileOpenDialog("i9001 Quick Flasher", @WorkingDir, "Image/Ext4 Files (*.img.ext4)", 1, "system.img.ext4")
			GUICtrlSetData($path_system, $getpath_system)

		Case $msg = $browse_custom
			$getpath_custom = FileOpenDialog("i9001 Quick Flasher", @WorkingDir, "Image/Ext4 Files (*.img.ext4)", 1, "*.img.ext4")
		...
		Case $msg = $start
			...
			GUICtrlSetData($log, "-------------------------------------------" & @CRLF & "i9001 Quick Flasher v1.4")
			_AddLog("Coded by broodplank1337 @ XDA")
			_AddLog("-------------------------------------------")
...
Func _Flashbootimg()
	...
	If BitAND(GUICtrlRead($wipe_bootimg), $GUI_Checked) = $GUI_Checked Then
		GUICtrlSetData($progress, 40)
		RunWait(@ScriptDir & "\adb.exe -d shell su -c " & Chr(34) & "dd if=/dev/zero of=/dev/block/mmcblk0p8" & Chr(34), "", @SW_HIDE)
		_AddLog("[X] Wipe /mmcblk0p8")
		GUICtrlSetData($progress, 50)
	EndIf
	GUICtrlSetData($progress, 70)
	RunWait(@ScriptDir & "\adb.exe -d shell su -c " & Chr(34) & "dd if=/sdcard/boot.img of=/dev/block/mmcblk0p8" & Chr(34), "", @SW_HIDE)
...
Func _Flashrecovery()
	...
	If BitAND(GUICtrlRead($wipe_recovery), $GUI_Checked) = $GUI_Checked Then
		GUICtrlSetData($progress, 40)
		RunWait(@ScriptDir & "\adb.exe -d shell su -c " & Chr(34) & "dd if=/dev/zero of=/dev/block/mmcblk0p13" & Chr(34), "", @SW_HIDE)
		_AddLog("[X] Wipe /mmcblk0p13")
		GUICtrlSetData($progress, 50)
	EndIf
	GUICtrlSetData($progress, 70)
	RunWait(@ScriptDir & "\adb.exe -d shell su -c " & Chr(34) & "dd if=/sdcard/recovery.img of=/dev/block/mmcblk0p13" & Chr(34), "", @SW_HIDE)
...
Func _Flashsystem()
	...
	If BitAND(GUICtrlRead($wipe_system), $GUI_Checked) = $GUI_Checked Then
		GUICtrlSetData($progress, 50)
		RunWait(@ScriptDir & "\adb.exe -d shell su -c " & Chr(34) & "dd if=/dev/zero of=/dev/block/mmcblk0p15" & Chr(34), "", @SW_HIDE)
	...
	RunWait(@ScriptDir & "\adb.exe -d shell su -c " & Chr(34) & "dd if=/sdcard/system.img.ext4 of=/dev/block/mmcblk0p15" & Chr(34), "", @SW_HIDE)
